...
Spring Boot, Spring Security, JWT를 활용한 권한별 차등 기능을 제공하는 은행 시스템 백엔드입니다.
...
```
com.example
├── auth          # 사용자 인증 (UserDetails)
├── config        # Security 설정, 예외 처리
...
| Bank_Account | id, accountNumber, balance, frozen, member_id(FK) |
...
| ADMIN | 전체 회원 목록 조회, 전체 계좌 잔액 조회, 계좌 동결/해제 |
...
| GET | /api/admin/accounts | 전체 회원 계좌 및 잔액 목록 | ADMIN |
...
- ADMIN 로그인 후 전체 회원 계좌 목록 및 잔액 총합 조회 성공
